...
public interface ServiceRequestRepository extends JpaRepository<ServiceRequest, Long> {
     ...
     *
     * <p>{@code studentId} and {@code departmentId} are not user-supplied
     * filters — they are how visibility is enforced. A student's own id is
     * pushed in here so the database never returns anyone else's rows, rather
     * than fetching everything and filtering in Java where a mistake leaks data.
     *
     * <p>The joins are fetched because every row on screen shows the category and
     * the reporter's name. Without them this would be the N+1 problem again, at
     * twenty rows a page.
     */
    @Query(value = """
